...
function RdpClient(config) {
  ...
  // credentials
  if (config.domain) {
    this.sec.infos.obj.domain.value = new Buffer(
      config.domain + "\x00",
      "ucs2"
    );
  }
  if (config.userName) {
    this.sec.infos.obj.userName.value = new Buffer(
      config.userName + "\x00",
      "ucs2"
    );
  }
  if (config.password) {
    this.sec.infos.obj.password.value = new Buffer(
      config.password + "\x00",
      "ucs2"
    );
  }
  if (config.workingDir) {
    this.sec.infos.obj.workingDir.value = new Buffer(
      config.workingDir + "\x00",
      "ucs2"
    );
  }
  if (config.alternateShell) {
    this.sec.infos.obj.alternateShell.value = new Buffer(
      config.alternateShell + "\x00",
      "ucs2"
    );
  }
  ...
  log.debug(
    "screen " +
      this.mcs.clientCoreData.obj.desktopWidth.value +
      "x" +
      this.mcs.clientCoreData.obj.desktopHeight.value
  );
  ...
  switch (config.locale) {
    case "fr":
      ...
      this.mcs.clientCoreData.obj.kbdLayout.value =
        t125.gcc.KeyboardLayout.FRENCH;
      break;
    case "en":
    default:
  ...
  this.global
    ...
    .on("bitmap", function (bitmaps) {
      for (var bitmap in bitmaps) {
        ...
        var isCompress =
          bitmaps[bitmap].obj.flags.value &
          pdu.data.BitmapFlag.BITMAP_COMPRESSION;
 ...
 */
RdpClient.prototype.sendKeyEventScancode = function (
  code,
  isPressed,
  extended
) {
 ...
 */
RdpClient.prototype.sendWheelEvent = function (
  x,
  y,
  step,
  isNegative,
  ...
  if (isNegative) {
    event.obj.pointerFlags.value |=
      pdu.data.PointerFlag.PTRFLAGS_WHEEL_NEGATIVE;
  }
